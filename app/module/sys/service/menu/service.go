...
import (
	...
	"github.com/ddh-open/gin/app/module/sys/model/menu"
...
func NewService(c framework.Container) *Service {
	...
	return &Service{base.NewRepository(db, &menu.DevopsSysMenu{})}
}
...
func (s *Service) GetMenuById(id string, grpcService contract.ServiceGrpc) ([]map[string]interface{}, error) {
	...
	defer conn.Close()
	client := userGrpc.NewServiceMenuClient(conn)
	resp, err := client.MenuList(context.Background(), &userGrpc.ListRequest{
		Filter: []string{"id = ?", id},
	})
	if err != nil {
		return result, err
...
func (s *Service) GetMenuList(request base.PageRequest, grpcService contract.ServiceGrpc, param ...interface{}) (base.PageResult, error) {
	conn, err := grpcService.GetGrpc("grpc.user")
...
func (s *Service) AddMenu(mapData map[string]interface{}, grpcService contract.ServiceGrpc, param ...interface{}) error {
	conn, err := grpcService.GetGrpc("grpc.user")
	if err != nil {
		return err
	}
	...
	client := userGrpc.NewServiceMenuClient(conn)
	resp, err := client.MenuAdd(context.Background(), &userGrpc.BytesRequest{
		Data: data,
	})
	if err != nil {
		return err
...
func (s *Service) ModifyMenu(mapData map[string]interface{}, grpcService contract.ServiceGrpc, param ...interface{}) error {
	conn, err := grpcService.GetGrpc("grpc.user")
	if err != nil {
		return err
	}
	...
	client := userGrpc.NewServiceMenuClient(conn)
	resp, err := client.MenuModify(context.Background(), &userGrpc.BytesRequest{
		Data: data,
	})
	if err != nil {
		return err
...
func (s *Service) DeleteMenu(ids string, grpcService contract.ServiceGrpc, param ...interface{}) error {
	var idsInt []int64
	...
	defer conn.Close()
	client := userGrpc.NewServiceMenuClient(conn)
	resp, err := client.MenuDelete(context.Background(), &userGrpc.IdsRequest{
		Ids: idsInt,
	})
	if err != nil {
		return err
