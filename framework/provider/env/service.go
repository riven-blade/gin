...
// NewNiceEnv 有一个参数，.env文件所在的目录
// example: NewNiceEnv("/envfolder/") 会读取文件: /envfolder/.env
// .env的文件格式 FOO_ENV=BAR
func NewNiceEnv(params ...interface{}) (interface{}, error) {
	if len(params) != 1 {
		return nil, errors.New("NewNiceEnv param error")
	}
...
func (en *NiceEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *NiceEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *NiceEnv) Get(key string) string {
...
func (en *NiceEnv) All() map[string]string {
	return en.maps
}
